...
%             fiveTowFrame = [fiveTowFrame bin2dec(subframe(31:47)) * 6 - 30];
            ...
            eph.T_GD        = [eph.T_GD (twosComp2dec(subframe(197:204)) * 2^(-31))];
            eph.IODC        = [eph.IODC (bin2dec([subframe(83:84) subframe(197:204)]))];
            eph.t_oc        = [eph.t_oc (bin2dec(subframe(219:234)) * 2^4)];
            eph.a_f2        = [eph.a_f2 (twosComp2dec(subframe(241:248)) * 2^(-55))];
            eph.a_f1        = [eph.a_f1 (twosComp2dec(subframe(249:264)) * 2^(-43))];
            eph.a_f0        = [eph.a_f0 twosComp2dec(subframe(271:292)) * 2^(-31)];
            idValid(1)  = 1;
...
%             fiveTowFrame = [fiveTowFrame bin2dec(subframe(31:47)) * 6 - 30];
            ...
            eph.M_0         = [eph.M_0 (twosComp2dec([subframe(107:114) subframe(121:144)])* 2^(-31) * gpsPi)];
            eph.C_uc        = [eph.C_uc (twosComp2dec(subframe(151:166)) * 2^(-29))];
            eph.e           = [eph.e (bin2dec([subframe(167:174) subframe(181:204)])* 2^(-33))];
            eph.C_us        = [eph.C_us (twosComp2dec(subframe(211:226)) * 2^(-29))];
            eph.sqrtA       = [eph.sqrtA (bin2dec([subframe(227:234) subframe(241:264)])* 2^(-19))];
            eph.t_oe        = [eph.t_oe (bin2dec(subframe(271:286)) * 2^4)];
            idValid(2)  = 2;
...
%             fiveTowFrame = [fiveTowFrame bin2dec(subframe(31:47)) * 6 - 30];
            ...
            eph.i_0         = [eph.i_0 (twosComp2dec([subframe(137:144) subframe(151:174)])* 2^(-31) * gpsPi)];
            eph.C_rc        = [eph.C_rc (twosComp2dec(subframe(181:196)) * 2^(-5))];
            eph.omega       = [eph.omega (twosComp2dec([subframe(197:204) subframe(211:234)])* 2^(-31) * gpsPi)];
            eph.omegaDot    = [eph.omegaDot (twosComp2dec(subframe(241:264)) * 2^(-43) * gpsPi)];
            eph.IODE_sf3    = [eph.IODE_sf3 (bin2dec(subframe(271:278)))];
            eph.iDot        = [eph.iDot (twosComp2dec(subframe(279:292)) * 2^(-43) * gpsPi)];
            idValid(3)  = 3;
...
eph.idValid=[eph.idValid; idValid];
