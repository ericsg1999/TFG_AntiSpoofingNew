...
% SCB
numSbcChecks=settings.msToProcess/settings.SbcPeriod;
%SCB            = zeros(1, numSbcChecks);
SCB=[];
%--- Copy initial settings for all channels -------------------------------
numberChannels=sum(size(channel.PRN,1));
% numTrackedSatellites=numberChannels/settings.AptNumberChannelsPerSat;
% trackResults = repmat(trackResults, 1, numTrackedSatellites);
trackResults = repmat(trackResults, 1, numberChannels);
...
for channelNr = 1:numberChannels
    if channel(channelNr).PRN==23
        switch channel(channelNr).status
            case 'APT'
                it=0;
                spoofingRecord=[];
                secondaryPeakMagnitudeVector=[];
                for loopCnt =  0:settings.AptPeriod:codePeriods
                    %% GUI update -------------------------------------------------------------
                ...
                %% APT
                    ...
                    [fileID,spoofingRecord,secondaryPeakMagnitudeVector] = APT_detection_check(settings,fileID,filePos,channel(channelNr).PRN,openFile,spoofingRecord,secondaryPeakMagnitudeVector);
                    it=it+1;
                end
                negativeAptDetectionsVector=find(spoofingRecord==-1);
                positiveAptDetectionsVector=find(spoofingRecord==1);

                negativeAptDetections=size(negativeAptDetectionsVector,2);
                positiveAptDetections=size(positiveAptDetectionsVector,2);
                totalAptChecks=size(spoofingRecord,2);

                probFalseAlarm=positiveAptDetections/totalAptChecks;
                probDetection=positiveAptDetections/totalAptChecks;

                disp(['------APT probabilities for PRN: ', num2str(channel(channelNr).PRN),' with Apt Threshold=  ',num2str(settings.AptThreshold), '----------'])
                disp(['Probability of False Alarm = ', num2str(probFalseAlarm), ': Probability of Detection= ',num2str(probDetection)])

                trackResults(channelNr).PRN     = channel(channelNr).PRN;
                trackResults(channelNr).status  = channel(channelNr).status;
    %             for loopCnt =  1:codePeriods %1:settings.AptPeriod:codePeriods
    %                 %% GUI update -------------------------------------------------------------
    %                 % The GUI is updated every 50ms. This way Matlab GUI is still
    %                 % responsive enough. At the same time Matlab is not occupied
    %                 % all the time with GUI task.
            ...
            case 'T'
                ...
                if (channel(channelNr).PRN ~= 0)
                    ...
                    for loopCnt =  1:codePeriods
                        ...
                        %% SCB spoofing detection check
                        if settings.SbcActive==1

                            if mod(loopCnt,settings.SbcPeriod)==0
                                SCB= [SCB I_E^2+Q_E^2-(I_L^2+Q_L^2)];
                            end
